...
function runInherit(cmd, args, options = {}) {
        ...
        if (res.error) {
                ...
                if (res.error.code === 'ETIMEDOUT') {
                        fail(
                                `命令 "${cmd} ${args.join(' ')}" 执行超时（>${options.timeout}ms），已终止`
                        )
                }
...
function resolveCliSource() {
        ...
        if (USE_LOCAL_CLI) {
                ...
                if (!existsSync(LOCAL_CLI_DIR)) {
                        fail(
                                `本地 CLI 源码目录不存在：${LOCAL_CLI_DIR}（请检查 LOCAL_CLI_DIR 常量）`
                        )
                }
        ...
        if (setHeadStatus !== 0) {
                fail(
                        `git remote set-head origin -a 失败（退出码 ${setHeadStatus}），无法确定远端默认分支`
                )
        }
...
function probePyInstaller() {
        ...
        if (res.error) {
                console.error(
                        `${LOG_PREFIX} 无法执行 Python 解释器 "${pythonCmd}"：${res.error.message}`
                )
...
function main() {
        log(`仓库根目录：${repoRoot}`)
        log(
                `平台映射：platformDir="${platformDir}" · exeName="${exeName}" · pythonCmd="${pythonCmd}"`
        )
        validateTarget()
